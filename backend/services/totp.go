...
func SetupTOTP(userID int64, username string) (secret string, otpauthURI string, qrBase64 string, err error) {
	// 生成随机 20 字节密钥
	key := make([]byte, 20)
	...
	return secret, otpauthURI, qrBase64, nil
}
...
func VerifyTOTPSetup(userID int64, code string) ([]string, error) {
	...
	// 验证通过 → 启用 TOTP
	now := time.Now().UTC().Format(time.RFC3339)
	_, err = db.DB.Exec(`UPDATE users SET totp_enabled = 1, totp_created_at = ? WHERE id = ?`, now, userID)
	if err != nil {
...
func DisableTOTP(userID int64) error {
	...
	_, err = tx.Exec(`UPDATE users SET totp_secret = '', totp_enabled = 0, totp_created_at = NULL WHERE id = ?`, userID)
	if err != nil {
		return err
	}
...
// AdminEnableTOTP 管理员强制启用用户的 TOTP（若无私钥则自动生成）
func AdminEnableTOTP(userID int64) error {
	var secret string
	err := db.DB.QueryRow(`SELECT COALESCE(totp_secret,'') FROM users WHERE id = ?`, userID).Scan(&secret)
	if err != nil {
		return fmt.Errorf("get user totp: %w", err)
	}
	...
	now := time.Now().UTC().Format(time.RFC3339)
	_, err = db.DB.Exec(`UPDATE users SET totp_secret = ?, totp_enabled = 1, totp_created_at = ? WHERE id = ?`, secret, now, userID)
	if err != nil {
		return fmt.Errorf("enable totp: %w", err)
	}

	// 清除旧恢复码
	db.DB.Exec(`DELETE FROM totp_recovery_codes WHERE user_id = ?`, userID)
...
func VerifyRecoveryCode(userID int64, code string) error {
	...
	for rows.Next() {
		...
		if bcrypt.CompareHashAndPassword([]byte(codeHash), []byte(code)) == nil {
			// 匹配成功 → 标记使用
			now := time.Now().UTC().Format(time.RFC3339)
			db.DB.Exec(`UPDATE totp_recovery_codes SET used = 1, used_at = ? WHERE id = ?`, now, id)
			return nil
		}
	}
	return fmt.Errorf("无效的恢复码")
